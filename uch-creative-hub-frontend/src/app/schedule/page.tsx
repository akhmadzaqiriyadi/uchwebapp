...
import { Loader2, Calendar as CalendarIcon, ServerCrash, ChevronLeft, ChevronRight, Clock, User, ArrowRight, Building } from "lucide-react";
...
export default function SchedulePage() {
  ...
  const { data: bookings, isLoading, isError, error } = useQuery<ScheduleItem[]>({
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 sm:py-8 lg:py-12">
        ...
        <div className="text-center md:pt-16 pt-24 mb-12">
          ...
          <h1 className="text-2xl sm:text-3xl lg:text-3xl font-bold text-slate-800 mb-3 sm:mb-4 lg:mb-4 px-2">Jadwal Publik UCH</h1>
          <p className="text-sm sm:text-base lg:text-lg text-slate-600 max-w-2xl mx-auto px-4 leading-relaxed">
            Lihat jadwal penggunaan ruangan yang telah disetujui untuk merencanakan aktivitas Anda.
          </p>
        </div>
        ...
        <Card className="shadow-lg sm:shadow-xl rounded-xl sm:rounded-2xl lg:rounded-2xl overflow-hidden bg-white/95 backdrop-blur-sm border-0 gap-0 py-0 mb-8 sm:mb-8 lg:mb-12">
          <CardHeader className="p-3 sm:p-4 lg:p-8 border-b border-slate-200/60">
            <div className="flex items-center justify-between gap-2 sm:gap-4">
              <Button onClick={() => navigateWeek("prev")} variant="outline" className="rounded-lg h-9 w-9 p-0 sm:h-10 sm:w-10 lg:h-10 lg:w-auto lg:px-4 lg:gap-2 flex-shrink-0">
              ...
              <h2 className="text-sm sm:text-base lg:text-2xl font-bold text-slate-800 text-center flex-1 px-2">
                {format(currentWeek, "d MMM", { locale: id })} - {format(addDays(currentWeek, 6), "d MMM yyyy", { locale: id })}
              </h2>
              <Button onClick={() => navigateWeek("next")} variant="outline" className="rounded-lg h-9 w-9 p-0 sm:h-10 sm:w-10 lg:h-10 lg:w-auto lg:px-4 lg:gap-2 flex-shrink-0">
          ...
          <CardContent className="p-2 sm:p-3 lg:p-8">
            <div className="grid grid-cols-7 gap-1 sm:gap-2">
              {weekDays.map((day) => (
                ...
                >
                  <p className="text-xs sm:text-sm font-medium truncate">{format(day, "EEE", { locale: id })}</p>
                  <p className="text-base sm:text-lg lg:text-2xl font-bold">{format(day, "d")}</p>
                </button>
              ))}
            </div>
          </CardContent>
        ...
        <div>
          {isLoading && (
            <div className="text-center p-8 sm:p-12 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-sm sm:text-base text-slate-600 font-medium">Memuat jadwal...</p>
            </div>
          )}
          ...
          {isError && (
             <Alert variant="destructive" className="max-w-2xl mx-auto rounded-xl">
              ...
              <AlertTitle className="text-sm sm:text-base">Gagal Memuat Data</AlertTitle>
          ...
          {!isLoading && !isError && (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
              {rooms.map(room => {
                ...
                return (
                  <Card key={room} className="shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 hover:scale-[1.02] rounded-xl sm:rounded-2xl border-0 overflow-hidden bg-white/95 backdrop-blur-sm gap-0 py-0">
                    <CardHeader className="bg-gradient-to-br from-slate-50 to-blue-50/30 p-4 sm:p-5 lg:p-6 border-b border-slate-200/50 gap-1.5">
                      <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <CardTitle className="text-base sm:text-lg lg:text-2xl font-bold text-slate-800 truncate">{room}</CardTitle>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="p-4 sm:p-5 lg:p-6">
                      {roomBookings.length > 0 ? (
                        <ul className="space-y-3 sm:space-y-4">
                          {roomBookings.sort((a,b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime()).map((booking, index) => (
                            <li key={index} className="p-3 sm:p-4 rounded-lg sm:rounded-xl bg-gradient-to-r from-slate-50 to-blue-50/20 border border-slate-200/70">
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0 mb-2">
                                <div className="flex items-center gap-2 text-sm sm:text-base font-bold text-slate-700">
                                  ...
                                  <span>{formatAsLocalTime(booking.startTime)} - {formatAsLocalTime(booking.endTime)}</span>
                                </div>
                              </div>
                              <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-500">
                                <User className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                                <span className="truncate">Dipesan oleh {booking.user.name}</span>
                              </div>
                            </li>
                          ))}
                        </ul>
                        ...
                        <div className="text-center py-6 sm:py-8">
                          ...
                          <p className="text-sm sm:text-base text-slate-500 font-medium">Tidak ada jadwal</p>
                          <p className="text-xs sm:text-sm text-slate-400 mt-1">untuk ruangan ini</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
        ...
        <div className="text-center mt-10 sm:mt-12 lg:mt-12 pt-6 sm:pt-8 border-t border-slate-200/80">
          ...
          <Button asChild size="lg" className="h-11 sm:h-12 px-6 sm:px-8 text-sm sm:text-base bg-gradient-to-r from-[#2E417A] to-blue-700 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 group transform hover:scale-105">
            <Link href="/login">
              Login untuk Booking
